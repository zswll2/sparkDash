...
- **Real client address behind a reverse proxy** — `TRUST_PROXY` (comma-separated IPs/CIDRs) names the peers whose `X-Forwarded-For` may be trusted; the login lockout, the loopback trust check and the logs then use the real client address. Unset, the header is ignored entirely, so a directly reachable port cannot be forged around the lockout.
